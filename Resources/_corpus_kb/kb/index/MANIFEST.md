...
|---|---|---|---|
| `lessons.json` | 295 | 274 KB | Lesson catalog: cefr, themes, audio_path, audio_exists, gap_count, line_count |
...
| `themes.json` | 440 | 38 KB | theme → [lesson_ids] |
| `../drills/<id>.json` | 271 | 519 KB total | Pincer drill packs (one per lesson) |
...
- `lessons.json` (~70 K tokens)
- `themes.json` (~10 K tokens)
- `grammar.json` (~4 K tokens)
- `patterns.json` (~5 K tokens)
- `refs/*.md` (~5 K tokens)
- Total system context: ~95 K tokens. Fits comfortably.
...
- 24 dialogues skipped from drill generation (revision/grammar-only lessons or [OCR unreadable] placeholders)

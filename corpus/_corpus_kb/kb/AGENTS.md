...
This `kb/` is a Karpathy-style wiki of the **Assimil corpus** (English + French language-learning courses), curated as direct input to an AI agent that practices **oral expression** with the user. It was scaffolded by the `corpus-indexer` skill from documents in `corpus/` (markdown cache at `_md_cache/`).
...
audio: <relative-path-to-mp3>           # path under corpus/
